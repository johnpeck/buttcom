...
* TODO Add the rxchar subsystem [0/3]
** TODO Make log messages from the received character ISR have the rxchar subsystem tag
** TODO Regenerate the "watching the progress figure," as the subsystem tags will have changed
** TODO Add an entry in logger.c for the new system
* TODO Add the logreg? command to query the logger config register [0/3]
** TODO Regenerate the command array listing, as the array will have increased
** TODO Add a function in logger.h and logger.c to handle the command
** TODO Follow your "adding a new command" procedure to check the procedure
* TODO Make a script to generate the "watching the progress" figure [0/2]
** TODO Just types hello and gets the reply
