...
I set up a logging system to help me understand how commands were being processed.  It's inconvienient to have the command and logging interfaces share the same communication channel, but the Butterfly only has one USART.  Reading command replys back without getting confused by general log messages requires some commands to control how the logging system works.  Figure \ref{fig:termgrab} shows the logger's output with all systems enabled for logging at all levels.
